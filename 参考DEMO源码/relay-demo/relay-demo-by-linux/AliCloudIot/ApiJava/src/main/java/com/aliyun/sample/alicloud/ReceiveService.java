...
public class ReceiveService implements Runnable {
    ...
    private static String consumerGroupId = "DEFAULT_GROUP";//消息组Id，测试用的是测试组;

    //iotInstanceId：实例ID。若是2021年07月30日之前（不含当日）开通的公共实例，请填空字符串。
    private static String iotInstanceId = "YourIotInstanceId";//我的公共实例是2021.07.30前开通的，所以测试时，用 "" ，请用户根据实际填写

    //控制台服务端订阅中消费组状态页客户端ID一栏将显示clientId参数。
    //建议使用机器UUID、MAC地址、IP等唯一标识等作为clientId。便于您区分识别不同的客户端。
    private static String clientId = "000EC64A3027";//这里用了电脑网卡的MAC地址

    //${YourHost}为接入域名，请参见AMQP客户端接入说明文档。接入域名组成，{$uid}.iot-amqp.cn.{$regionId}.aliyuncs.com
    //uid其实就是你自己的账号Id，在登录入阿里云后，进入控制台，鼠标移到右上角的，在展开的信息中，就可以看到你的uid
    //公共实例或企业实例所在的服务器所处的区域，如果是 华东2(上海) ，即为 cn-shanghai
    //假如uid为:123456789987654321  "123456789987654321.iot-amqp.cn-shanghai.aliyuncs.com"
    private static String host = "{$uid}.iot-amqp.cn.{$regionId}.aliyuncs.com";//"${YourHost}"
    ...
    public void run() {
        ...
        for (int i = 0; i < connectionCount; i++) {
            ...
            try {
                consumer.setMessageListener(messageListener);//注意这里使用了回调
            } catch (JMSException e) {
                e.printStackTrace();
            }
        }
    ...
    private static MessageListener messageListener = new MessageListener() {
        ...
        public void onMessage(final Message message) {
            try {
                ...
                executorService.submit(new Runnable() {//通过线程池进行异步处理
                    @Override
                    public void run() {
